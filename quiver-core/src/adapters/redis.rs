...
mod tests {
    ...
    #[tokio::test]
    async fn test_pipelined_key_building_performance() {
        use std::time::Instant;

        let source_path = SourcePath::Template("{feature}:{entity}".to_string());

        // Mock adapter creation (won't actually connect)
        let mock_connection = match redis::Client::open("redis://localhost:6379") {
            Ok(client) => match client.get_multiplexed_async_connection().await {
                Ok(conn) => conn,
                Err(_) => {
                    println!("Skipping Redis test - Redis server not available");
                    return;
                }
            },
            Err(_) => {
                println!("Skipping Redis test - Redis client creation failed");
                return;
            }
        };
        ...
        let adapter = RedisAdapter {
            ...
            default_source_path: source_path.clone(),
            timeout_default: StdDuration::from_secs(5),
        ...
        // Test key building performance for multiple features and entities
        let feature_names = vec!["age", "country", "credit_score", "email"];
        let entity_ids = vec!["user1", "user2", "user3", "user4", "user5"];

        let start = Instant::now();

        // Build all keys like the optimized implementation does
        let mut all_keys = Vec::new();
        for feature_name in &feature_names {
            for entity_id in &entity_ids {
                let key = adapter
                    .build_key_unchecked(feature_name, entity_id, &source_path)
                    .unwrap();
                all_keys.push(key);
            }
        }
        ...
        // Performance characteristic: should build keys quickly
        assert!(
            build_time.as_millis() < 10,
            "Key building should complete in under 10ms, took: {:?}",
            build_time
        );

        // Verify correct number of keys
        assert_eq!(all_keys.len(), feature_names.len() * entity_ids.len());

        // Verify key format
        assert_eq!(all_keys[0], "age:user1");
        assert_eq!(all_keys[all_keys.len() - 1], "email:user5");

        println!(
            "✓ Built {} keys for pipelined access in {:?}",
            all_keys.len(),
            build_time
        );
    }
